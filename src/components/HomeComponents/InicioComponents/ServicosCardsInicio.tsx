    ...
    <main className="flex flex-col items-center justify-center gap-5 p-2">
      ...
      <article className="flex flex-wrap justify-center gap-4">
        {cardsInicio.map((item) => (
          <section
            key={item.key}
            className="hover:bg-tertiary/5 flex w-36 flex-col gap-4 rounded-2xl bg-white px-2 py-4 shadow-md transition-colors"
          >
            ...
            <h4 className="text-tertiary text-center text-xs font-medium">
              {item.label}
            </h4>
          </section>
        ))}
